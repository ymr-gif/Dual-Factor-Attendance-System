...
## Build status & where things live (Steps 10+)

**Docs map — read the right file, don't reload everything:**
- **`ROADMAP.md`** — the plan. Start at the "Build sequence" section (the single dependency-ordered path across five tracks: UI 10–16, Backbone 20–23, Flow 30–35, Deploy 40–44, Tuning 50–54).
- **`docs/build-log.md`** — detailed per-step build record (files, env knobs, verification). **Read the relevant entry before modifying a built area.**
- **`docs/design-notes.md`** — cross-cutting constraints, failure modes, correlation edge-case rules (incl. §5a adaptive/late-bind stub).
- **`handoff.md`** — active UI-only handoff (all 9 tasks built; Task 10 polish deferred).
- **`docs/verification.md`**, **`docs/face-verification.md`**, **`docs/privacy.md`** — test runbooks + data handling.

**Done:** Steps 10, 11, 12, 13, 14, 15, 20, 21, 30, 31, 33, 34, 54 + backend management layer (14/22/34/50) + the handoff UI surfaces + MJPEG stream (per-client fan-out) + register guided-capture/quality-gate + success-status/method labelling. **Partially done:** Step 35 (register wizard + boxes-only viewer built; **roles** remaining). **Next:** Step 16 (README screenshots, CI, CLAUDE.md sync) · Step 35 roles · Step 44 (release CI). GPU/HW acceptance (Step 30 throughput, 51/52/53, 40/41) waits for the RTX 1050 box.

**Architecture note (critical):** a major change is **built (Steps 30–31)** and **currently ON** (`PERCEPTION_ENABLED=true` in the backend unit; code default false). The Flow track replaces per-tap **1:1 verification** with a **continuous perception pipeline + async tap↔face correlation** (single camera owner, IoU tracking, Hungarian matcher) for a 3–5 students/sec guardpost. **Before writing code against the 1:1 flow, check whether the Flow track supersedes it** (see `docs/build-log.md` Steps 30–31). Non-negotiable constraints (design-notes): single camera owner, single backend worker (in-memory tap buffer/event bus), reader-throughput ceiling (one RC522 @ 9600 baud can't do 3–5/s), camera-dead = `unverified` + alert (not silent card-only), legal/consent + accuracy-eval gates for children's biometrics.
