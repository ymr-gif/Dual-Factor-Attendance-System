...
Cohesion: 0.17
Nodes (14): gpu_runtime(), (ctx_id, providers) honoring USE_GPU, with automatic CPU fallback.      Shared s, assess(), _crop(), _get_new_box(), get_sessions(), Passive liveness / anti-spoofing (Step 7). MiniFASNet ensemble via onnxruntime., Score liveness of the face at `bbox` in `frame`.      bbox: InsightFace [x1,y1,x (+6 more)
...
Cohesion: 0.28
Nodes (6): _check(), System health check (Step 22).  Usage:     python -m backend.doctor  Checks ever, run(), _load_reference(), main(), Live camera diagnostic UI (setup aid, not part of the tap flow).  Shows what the
