...
## Corpus Check
- 37 files · ~25,190 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 457 nodes · 592 edges · 29 communities (27 shown, 2 thin omitted)
...
Cohesion: 0.08
Nodes (31): main(), Threshold calibration helper (read-only).  Prints the distribution of logged sco, _report(), capture_probe(), detect(), embed(), encode_image(), get_app() (+23 more)

### Community 1 - "Liveness / Anti-Spoof (MiniFASNet)"
Cohesion: 0.08
Nodes (27): counts_as_present(), decide(), Attendance verification decision / buddy-punch enforcement (Step 9).  NFC identi, Collapse the per-factor verdicts into one attendance status.      student:, True when the tap should count as attendance. Rejected + the matcher review, gpu_runtime(), (ctx_id, providers) honoring USE_GPU, with automatic CPU fallback.      Shared s, main() (+19 more)

### Community 2 - "Tap Endpoint & Notify"
Cohesion: 0.08
Nodes (24): api_set_consent(), ConsentRequest, _post_log(), Matcher outcome writer (Step 31): log -> notify -> broadcast. Fail-open., notify + broadcast for a synchronously-written log (fail-open on both)., Guard /api/* endpoints. Accepts `Authorization: Bearer <t>` or     `X-Operator-T, require_operator(), _strip_embedding() (+16 more)
...
Cohesion: 0.06
Nodes (33): Backbone track (Steps 20–23), Config precedence (new), Conflicts this track resolves (current code that must change), Flow-track deferred items (need GPU box / live cam / kiosk), Flow track (Steps 30–35) — continuous multi-student guardpost, How to use this doc, Locked decisions, Locked decisions (from design review) (+25 more)
...
Cohesion: 0.12
Nodes (26): delete_student(), find_student_by_uid(), get_attendance(), get_audit(), get_conn(), get_stats_today(), get_student(), get_students() (+18 more)
...
## Knowledge Gaps
- **166 isolated node(s):** `AbstractEventLoop`, `Any`, `WebSocket`, `name`, `private` (+161 more)
